...
from core.yolov4 import YOLO, compute_loss, decode, decode_train
from core.utils import freeze_all, unfreeze_all
from core.dataset import Dataset, DatasetV2
...
def main(_argv):
    ...
    trainset = DatasetV2(FLAGS, is_training=True).get_tf_dataset(
        cfg.TRAIN.FISRT_STAGE_EPOCHS + cfg.TRAIN.SECOND_STAGE_EPOCHS)
    testset = DatasetV2(FLAGS, is_training=False).get_tf_dataset(
        cfg.TRAIN.FISRT_STAGE_EPOCHS + cfg.TRAIN.SECOND_STAGE_EPOCHS)
    ...
    # Outputs several feature maps, each making prediction at a different scale
    ...
    # Restore model weights from weights file/checkpoint
    if FLAGS.weights == None:
        print("Training from scratch")
    else:
        if FLAGS.weights.split(".")[-1] == "weights":
    ...
    for epoch in range(first_stage_epochs + second_stage_epochs):
        ...
        # model.save_weights("./saved_weights/yolov4")

    # Export final model
    model.save("./saved_models/yolov4", save_format="tf")
